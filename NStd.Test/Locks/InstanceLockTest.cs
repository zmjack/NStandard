...
namespace NStd.Locks.Test
{
    public class InstanceLockTest
    {
        ...
        public void Test()
        {
            var lockParser = new InstanceLockParser<Model>(nameof(NStd), x => x.Year, x => x.Month, x => x.Sex, x => "const");
        ...
        {
            Assert.ThrowsAny<ArgumentException>(() => new InstanceLockParser<Model>(nameof(NStd), x => x.Year, x => x.Obj));
        }
